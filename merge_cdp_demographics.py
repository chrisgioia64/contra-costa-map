#!/usr/bin/env python3
"""
Script to merge demographic data from CSV into GeoJSON features.
Matches GeoJSON NAMELSAD property to CSV City_Name (or CDTFA_CITY) column.
"""
...
def load_csv(filepath):
    ...
    with open(filepath, 'r', encoding='utf-8-sig') as f:  # utf-8-sig handles BOM
        ...
        city_column = None
        if 'City_Name' in cleaned_columns:
        ...
        elif 'CDTFA_CITY' in cleaned_columns:
            city_column = 'CDTFA_CITY'
            city_column_idx = cleaned_columns.index('CDTFA_CITY')
            city_column_original = original_columns[city_column_idx]
            print("Note: Using 'CDTFA_CITY' column instead of 'City_Name'")
        else:
            raise ValueError(f"Could not find 'City_Name' or 'CDTFA_CITY' column in CSV. Available columns: {cleaned_columns}")
...
def merge_demographics(geojson_data, demographics_dict):
    ...
    if unmatched_count > 0 and unmatched_count <= 20:
        print(f"\nUnmatched place names (first 20):")
        for name in unmatched_names[:20]:
            print(f"    - {name}")
    elif unmatched_count > 20:
        print(f"\nUnmatched place names (showing first 20 of {unmatched_count}):")
        for name in unmatched_names[:20]:
            print(f"    - {name}")
